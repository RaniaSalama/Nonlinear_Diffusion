...
node1, node2 = findnz(triu(A));
m = length(node1);
em = ones(m);
edges_idx = collect(1:m);
N = sparse(vcat(edges_idx,edges_idx),[node1; node2], vcat(em, -1 * em));
...
for numcom = 1:totalcommunities
    ...
    conds = zeros(deg, 1)
    for trial = 1:deg
        bset, conds[trial, 1] = NonlinearDiffusion.run_nnlinear_diffusion(A,N, pinvD, h, verts[trial], t, p, truncCutoff)
