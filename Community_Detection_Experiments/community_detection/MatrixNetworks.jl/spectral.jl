...
immutable SweepcutProfile{V,F}
    ...
    total_volume::V
    total_nodes::Int
	number_of_nnz_nodes::Int

    function SweepcutProfile(p::Vector{Int},nnodes::Int,totalvol::V)
        n = length(p)
        new(p,Array(F,n-1),Array(V,n-1),Array(V,n-1),totalvol,nnodes)
    end
    function SweepcutProfile(p::Vector{Int},nnodes::Int,totalvol::V, conductance::Array{Float64}, cut::Array{Float64}, volume::Array{Float64}, nnz::Int)
        n = length(p)
        new(p,conductance, cut, volume,totalvol,nnodes,nnz)
    end
end
...
~~~~
"""
function sweepcut{V,T}(A::SparseMatrixCSC{V,Int}, p::Vector{Int}, r, totalvol::V, maxvol::T, x::Vector{T})
    ...
    for (i,v) in enumerate(p)
	    #@printf("%f\n", x[v])
	    if x[v] < 1e-12
			output = SweepcutProfile{V,F}(p,n,totalvol,output.conductance,output.cut,output.volume,i-1)
			break
		end
...
function sweepcut{V,T}(A::SparseMatrixCSC{V,Int}, x::Vector{T}, vol::V)
    ...
    r = RankedArray(ranks)
    return sweepcut(A, p, r, vol, Inf, x)
end
...
function bestset{V,F}(prof::SweepcutProfile{V,F})
    ...
    if !isempty(prof.conductance)
		bsetind = indmin(prof.conductance[1:prof.number_of_nnz_nodes])
