...
sigma_cora = np.loadtxt( 'sigma_change_cora_power.txt', delimiter=',')
plt.plot(sigma_cora[:,0], sigma_cora[:,1], '--r', label='Cora', linewidth=10, markersize=10)

sigma_citeseer = np.loadtxt( 'sigma_change_citeseer_power.txt', delimiter=',')
...
sigma_pubmed = np.loadtxt( 'sigma_change_pubmed_power.txt', delimiter=',')
...
plt.ylim([40, 90])
#plt.show()
plt.savefig("stability_power_sigma.eps")
