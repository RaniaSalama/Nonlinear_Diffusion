...
sigma_cora = np.loadtxt( 'p1_change_cora.txt', delimiter=',')
plt.plot(sigma_cora[:,0], sigma_cora[:,1], '--r', label='Cora', linewidth=10, markersize=10)

sigma_citeseer = np.loadtxt( 'p1_change_citeseer.txt', delimiter=',')
...
sigma_pubmed = np.loadtxt( 'p1_change_pubmed.txt', delimiter=',')
...
plt.xlabel('p1', fontsize=30)
plt.ylim([40, 90])
#plt.show()
plt.savefig("stability_power_p1.eps")
