...
outf = open("p1_change_"+args.dataset+".txt", "w")


for p1 in np.arange(0.4, 0.7, 0.01):
	if args.use_two_diffusions == 0:
		...
		if args.self_learning == 0:
			preds = calculate_two_nonlinear_diffusions(idx_train , y, args.t, args.h, p1, args.p2, LF, Ladj, args.sigma, pinvDF, pinvDadj, args.w, n, nclass, args.function)
		else:
			preds = self_learning(idx_train , y, args.t, args.h, p1, args.p2, LF, Ladj, args.sigma, pinvDF, pinvDadj, args.w, n, nclass, y, idx_test, args.rank_based, args.function)
	...
	if args.rank_based == 0:
		...
		outf.write(str(p1)+","+str(accuracy)+"\n")
	else:
